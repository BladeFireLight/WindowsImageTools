...
function Set-UpdateConfig
{
    ...
    (
        ...
        [ValidateScript({if (test-path $_) {$true} else {throw "Path $_ does not exist"}})]
        [Alias('FullName')] 
        $Path,
        ...
        [ValidateScript({ $ipObj = [System.Net.IPAddress]::parse($_)
                          $isValidIP = [System.Net.IPAddress]::tryparse([string]$_, [ref]$ipObj)
                          if ($isValidIP) {$true} 
                          else {throw 'IpAddress must be a valid IPv4 or IPv6 address'}})]
        [String]
        $IpAddress,
        ...
        [ValidateScript({ $ipObj = [System.Net.IPAddress]::parse($_)
                          $isValidIP = [System.Net.IPAddress]::tryparse([string]$_, [ref]$ipObj)
                          if ($isValidIP) {$true} 
                          else {throw 'Gateway must be a valid IPv4 or IPv6 address'}})]
        [String]
        $Gateway,
        ...
        [ValidateScript({ $ipObj = [System.Net.IPAddress]::parse($_)
                          $isValidIP = [System.Net.IPAddress]::tryparse([string]$_, [ref]$ipObj)
                          if ($isValidIP) {$true} 
                          else {throw 'DNSServer must be a valid IPv4 or IPv6 address'}})]
        [String]
        $DnsServer
    ...
    if ($pscmdlet.ShouldProcess("$Path", 'Set the Windows Image Tools Update Configuration'))
    {
      ...
      {
        Write-Warning "Unable to read Windows Image Tools Update Cofniguration from $ConFigFilePath, creating a new file"
        $ConfigData =  @{VmSwitch = 'vmswitch'; vLan = 0; IpAddress = '192.168.0.100'; SubnetMask = 24; Gateway = '192.168.0.1'; DnsServer = '192.168.0.1' ; IpType = 'DHCP' }
      }
      # validate data structure incase useing older or malformed xml
      If (-not ($ConfigData.ContainsKey('VmSwitch'))) { $ConfigData.add('VmSwitch','vmswitch') }
      If (-not ($ConfigData.ContainsKey('vLan'))) { $ConfigData.add('vLan','0') }
      If (-not ($ConfigData.ContainsKey('IpType'))) { $ConfigData.add('IpType','DHCP') }
      If (-not ($ConfigData.ContainsKey('IpAddress'))) { $ConfigData.add('IpAddress','192.168.0.100') }
      If (-not ($ConfigData.ContainsKey('SubnetMask'))) { $ConfigData.add('SubnetMask','24') }
      If (-not ($ConfigData.ContainsKey('Gateway'))) { $ConfigData.add('Gateway','192.168.0.1') }
      If (-not ($ConfigData.ContainsKey('DnsServer'))) { $ConfigData.add('DnsServer','192.168.0.1') }
      
      # update values
      if ($VmSwitch) {$ConfigData.VmSwitch = $VmSwitch}
      if ($vLan) {$ConfigData.vLan = $vLan}
      if ($IpType) {$ConfigData.IpType = $IpType}
      if ($IpAddress) {$ConfigData.IpAddress = $IpAddress}
      if ($SubnetMask) {$ConfigData.SubnetMask = $SubnetMask}
      if ($Gateway) {$ConfigData.Gateway = $Gateway}
      if ($DnsServer) {$ConfigData.DnsServer = $DnsServer}
...
function Get-UpdateConfig
{
    ...
    return (Import-Clixml "$Path\config.xml")
}

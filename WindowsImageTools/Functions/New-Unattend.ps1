          ...
          if ($setting.'Pass' -eq 'specialize' -and $component.'Name' -eq 'Microsoft-Windows-UnattendedJoin' )
          {
            if (($JoinAccount) -or ($domain) -or ($OU))
            {
              if (($JoinAccount) -and ($domain) -and ($OU))
              {
                ...
                $IdCredDomainTextNode = $IdCredDomainEliment.AppendChild($unattendXml.CreateTextNode($domain))
                ...
                $IdCredPasswordTextNode = $IdCredPasswordElement.AppendChild($unattendXml.CreateTextNode($JoinAccount.GetNetworkCredential().Password))
                ...
                $IdCredUserNameTextNode = $IdCredUserNameElement.AppendChild($unattendXml.CreateTextNode($JoinAccount.GetNetworkCredential().UserName))
                ...
                $IdJoinDomainTextNode = $IdJoinDomainElement.AppendChild($unattendXml.CreateTextNode($domain))
                ...
                $IdJoinDomainTextNode = $IdMachineOUElement.AppendChild($unattendXml.CreateTextNode($OU))
                ...
                $IdUnsecureJoinTextNode = $IdUnsecureJoinElement.AppendChild($unattendXml.CreateTextNode('False'))

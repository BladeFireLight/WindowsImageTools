        ...
        'Overwrite WARNING!'))
        {
            if($Force -Or $pscmdlet.ShouldContinue('', '')) 
            {
                ...
                try 
                {
                    ...
                    if ($Recovery)
                    { 
                        $cmd = "$windir\System32\reagentc.exe"
                        $args = " /setosimage /path $recoverfolder /index $Index /target $windir"
                        Write-Verbose -Message "[$($MyInvocation.MyCommand)] [$VhdxFileName] Partition [$partitionNumber] : $cmd"
                        #Invoke-Expression -Command $cmd
                        start-process -FilePath $cmd -ArgumentList $args -NoNewWindow -Wait -PassThru
